...
final class RarityBadgeView extends View {
 ...
 synchronized(ART){artwork=ART[this.tier];if(artwork==null){try(java.io.InputStream in=c.getAssets().open("media/rarities/v2/"+KEYS[this.tier]+".png")){BitmapFactory.Options options=new BitmapFactory.Options();options.inSampleSize=1;artwork=BitmapFactory.decodeStream(in,null,options);ART[this.tier]=artwork;}catch(java.io.IOException ignored){/* Original vector badge remains the fallback. */}}}}
