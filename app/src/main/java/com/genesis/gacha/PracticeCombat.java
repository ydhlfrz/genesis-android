...
final class PracticeCombat {
 ...
 void clearInput(){mx=my=0;attackHeld=skillPressed=dodgePressed=false;}
 ...
 void reset(){x=4;y=5;bx=11;by=5;fx=1;fy=0;clock=actionTime=skillCooldown=dodgeCooldown=0;
  ...
  action=bossPhase=pattern=0;playerHit=bossHit=false;outcome="";rng=163;paused=true;clearInput();}
 ...
 void tick(){
  ...
  else if(attackHeld&&action==0){faceMovement();action=1;actionTime=0;playerHit=false;}
  skillPressed=dodgePressed=false;
  ...
  if(bossPhase==0){double dx=x-bx,dy=y-by,d=Math.hypot(dx,dy);
   ...
   if(d<=2&&bossTime<=0){bossPhase=1;bossTime=pattern==0?.9:1.1;bossHit=false;}
  }else if(bossPhase==1&&bossTime<=0){bossPhase=2;bossTime=pattern==0?.10:.5;double d=Math.hypot(x-bx,y-by);chargeX=d<.001?1:(x-bx)/d;chargeY=d<.001?0:(y-by)/d;}
  ...
  if(toBoss>0){bossHp=Math.max(0,bossHp-toBoss);bossFlash=.15;}
  if(toPlayer>0){hp=Math.max(0,hp-toPlayer);hitFlash=.15;}
