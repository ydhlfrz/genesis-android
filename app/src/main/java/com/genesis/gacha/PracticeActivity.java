...
public final class PracticeActivity extends Activity {
 ...
 @Override public void onCreate(Bundle saved){super.onCreate(saved);
  ...
  attack.setOnTouchListener((v,e)->{if(e.getActionMasked()==MotionEvent.ACTION_DOWN){game.attackHeld=foreground&&!game.paused;v.setPressed(true);return true;}if(e.getActionMasked()==MotionEvent.ACTION_UP||e.getActionMasked()==MotionEvent.ACTION_CANCEL){game.attackHeld=false;v.setPressed(false);if(e.getActionMasked()==MotionEvent.ACTION_UP)v.performClick();return true;}return true;});
 ...
 private void showMenu(){
  game.pause();if(attack!=null)attack.setPressed(false);if(arena!=null)arena.releaseInput();if(menu!=null&&menu.isShowing())return;
  boolean ended=!game.outcome.isEmpty();
  menu=new AlertDialog.Builder(this).setTitle(ended?game.outcome:"Ruined Sanctum · Practice")
   .setMessage(ended?"Training session complete. No account rewards or penalties.":"Move with the left stick. Face the Warden to attack. Avoid the marked area before it strikes.\n\nTraining hero and graphics are placeholders.")
   .setPositiveButton(ended?"Retry":"Play",(d,w)->{if(ended)game.reset();arena.resetClock();arena.releaseInput();if(foreground)game.resume();})
 ...
 @Override protected void onPause(){foreground=false;game.pause();if(arena!=null)arena.stop();super.onPause();}
 @Override protected void onDestroy(){if(arena!=null)arena.stop();if(menu!=null)menu.dismiss();super.onDestroy();}
 ...
 private final class Arena extends View implements Choreographer.FrameCallback {
  ...
  @Override public void doFrame(long now){if(!running)return;
   ...
   else accumulator=0;last=now;labels();invalidate();
  ...
  @Override protected void onDraw(Canvas c){super.onDraw(c);c.drawColor(0xff111e25);
   ...
   if(game.bossPhase==1||game.bossPhase==2){
    ...
    else {double dx=game.bossPhase==1?game.x-game.bx:game.chargeX,dy=game.bossPhase==1?game.y-game.by:game.chargeY;double angle=Math.toDegrees(Math.atan2(dy,dx));c.save();c.rotate((float)angle,bx,by);color(danger);c.drawRect(bx,by-.6f,bx+4,by+.6f,paint);color(0xffffd887);c.drawRect(bx,by-.04f,bx+4,by+.04f,paint);c.restore();}
   }
   ...
   if(game.action==1||game.action==2){float r=game.action==1?1.3f:1.8f;float sweep=game.action==1?90:150;float angle=(float)Math.toDegrees(Math.atan2(game.fy,game.fx));paint.setColor(0xaa9aeeff);paint.setStyle(Paint.Style.STROKE);paint.setStrokeWidth(.09f);c.drawArc(px-r,py-r,px+r,py+r,angle-sweep/2,sweep,false,paint);paint.setStyle(Paint.Style.FILL);}
   c.restore();
